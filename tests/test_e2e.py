...
class TestSGLangStandaloneMode:
    """Tests for SGLang standalone mode (without Dynamo/NATS/etcd)."""

    RACK = H100Rack

    def test_sglang_frontend_skips_dynamo_env_vars(self):
        """SGLang frontend should not set Dynamo-specific environment variables."""
        ...
        # Mock config with sglang frontend
        @dataclass
        class MockFrontend:
            type: str = "sglang"
        ...
        @dataclass
        class MockConfig:
            frontend: MockFrontend = field(default_factory=MockFrontend)
            profiling: MagicMock = field(default_factory=lambda: MagicMock(enabled=False, is_nsys=False))
            backend: MagicMock = field(default_factory=MagicMock)

        @dataclass
        class MockNodes:
            head: str = "node-01"
            infra: str = "node-01"
            worker: list = field(default_factory=lambda: ["node-01", "node-02"])

        @dataclass
        class MockRuntime:
            head_node_ip: str = "10.0.0.1"
            nodes: MockNodes = field(default_factory=MockNodes)
            log_dir: MagicMock = field(default_factory=MagicMock)
            container_image: str = "test:latest"
            container_mounts: dict = field(default_factory=dict)
            gpus_per_node: int = 8
            environment: dict = field(default_factory=dict)

        # Create a minimal WorkerStageMixin to test env var logic
        class TestWorkerStage(WorkerStageMixin):
            def __init__(self):
            ...
            def backend(self):
                mock = MagicMock()
                mock.get_environment_for_mode.return_value = {}
                mock.get_process_environment.return_value = {}
                mock.build_worker_command.return_value = ["python", "-m", "test"]
                return mock
        ...
        worker_stage = TestWorkerStage()

        # Test that start_worker builds correct env vars
        mock_process = MagicMock(spec=Process)
        mock_process.endpoint_mode = "prefill"
        mock_process.endpoint_index = 0
        mock_process.node = "node-01"
        mock_process.sys_port = 8081
        mock_process.http_port = 30000
        mock_process.gpu_indices = [0, 1]

        # Build env vars manually (simulating what start_worker does)
        env_to_set = {"HEAD_NODE_IP": worker_stage.runtime.head_node_ip}

        # This is the key check - dynamo env vars should NOT be added for sglang frontend
        if worker_stage.config.frontend.type == "dynamo":
            env_to_set.update({
                "ETCD_ENDPOINTS": f"http://{worker_stage.runtime.nodes.infra}:2379",
                "NATS_SERVER": f"nats://{worker_stage.runtime.nodes.infra}:4222",
                "DYN_SYSTEM_PORT": str(mock_process.sys_port),
                "DYN_REQUEST_PLANE": "nats",
            })

        # Verify no Dynamo env vars are set for sglang frontend
        assert "ETCD_ENDPOINTS" not in env_to_set
        assert "NATS_SERVER" not in env_to_set
        assert "DYN_SYSTEM_PORT" not in env_to_set
        assert "DYN_REQUEST_PLANE" not in env_to_set
        assert "HEAD_NODE_IP" in env_to_set

    def test_dynamo_frontend_sets_dynamo_env_vars(self):
        """Dynamo frontend should set all Dynamo-specific environment variables."""
        from dataclasses import dataclass, field
        from unittest.mock import MagicMock
        ...
        # Build env vars as start_worker does
        env_to_set = {"HEAD_NODE_IP": "10.0.0.1"}

        if mock_frontend.type == "dynamo":
            env_to_set.update({
                "ETCD_ENDPOINTS": f"http://{mock_nodes.infra}:2379",
                "NATS_SERVER": f"nats://{mock_nodes.infra}:4222",
                "DYN_SYSTEM_PORT": "8081",
                "DYN_REQUEST_PLANE": "nats",
            })

        # Verify all Dynamo env vars are set
        assert "ETCD_ENDPOINTS" in env_to_set
        assert "NATS_SERVER" in env_to_set
        assert "DYN_SYSTEM_PORT" in env_to_set
        assert "DYN_REQUEST_PLANE" in env_to_set
        assert env_to_set["ETCD_ENDPOINTS"] == "http://node-01:2379"
        assert env_to_set["NATS_SERVER"] == "nats://node-01:4222"

    def test_sglang_backend_uses_correct_module(self):
        """SGLang backend should use sglang.launch_server for sglang frontend."""
        from dataclasses import dataclass, field
        from pathlib import Path
        from unittest.mock import MagicMock
        ...
        recipe_path = RECIPES_DIR / "qwen3-32b" / "agg-kv-sglang.yaml"
        if not recipe_path.exists():
            pytest.skip("agg-kv-sglang.yaml not found")

        with patch.dict(os.environ, self.RACK.slurm_env(), clear=False):
            with patch("subprocess.run", side_effect=self.RACK.mock_scontrol()):
                config = load_config(str(recipe_path))

                # Verify frontend is sglang
                assert config.frontend.type == "sglang"

                # Create a mock runtime to test command building
                @dataclass
                class MockRuntime:
                    model_path: Path = field(default_factory=lambda: Path("hf:Qwen/Qwen3-32B"))
                ...
                # Get first process and build command
                process = processes[0]
                ...
                # Verify it uses sglang.launch_server, not dynamo.sglang
                assert "sglang.launch_server" in cmd
                assert "dynamo.sglang" not in cmd

                # Verify --port is passed (sglang.launch_server uses --port, not DYN_SYSTEM_PORT)
                assert "--port" in cmd
